...
const btnGenerarListadoSimeGestion = document.getElementById(
  "btnGenerarListadoSimeGestion",
);
...
async function generarListadoSimePdfGestion() {
  const rolActual = String(window.portalUsuario?.rol || "")
    .trim()
    .toUpperCase();
  ...
  if (!rolActual) {
    await Swal.fire({
      ...
      text: "No se pudo determinar el rol activo de la sesión. Recargá el portal e intentá nuevamente.",
      icon: "warning",
  ...
  const textoOriginal = btnGenerarListadoSimeGestion.innerHTML;
  btnGenerarListadoSimeGestion.disabled = true;
  btnGenerarListadoSimeGestion.innerHTML = `
  ...
  try {
    ...
    const resultado = await enviarAlBackendListadoSime({
      accion: "obtener_datos_listado_pdf_gestion",
      idToken,
      rolActivo: rolActual,
    });
    ...
    if (!resultado.ok) {
      throw new Error(
        resultado.mensaje ||
          "No se pudieron consultar los datos del listado S.I.M.E.",
      );
    }
    ...
    btnGenerarListadoSimeGestion.disabled = false;
    btnGenerarListadoSimeGestion.innerHTML = textoOriginal;
  }
}

function configurarBotonListadoSimeGestion() {
  if (!btnGenerarListadoSimeGestion) return;

  // El botón queda disponible para cualquier usuario que haya ingresado
  // correctamente al Portal Gestión, sin restringirlo a un rol específico.
  btnGenerarListadoSimeGestion.hidden = !window.portalUsuario;
}

window.addEventListener(
  "portalUsuarioListo",
  configurarBotonListadoSimeGestion,
);
...
if (btnGenerarListadoSimeGestion) {
  btnGenerarListadoSimeGestion.addEventListener(
    "click",
    generarListadoSimePdfGestion,
  );
}
